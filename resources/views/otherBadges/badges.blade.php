                ...
                <div class="mb-4">
                    ...
                    <select name="badge_type" id="badge_type" class="form-select">
                        ...
                        @foreach($types as $type)
                            @if($type->name != "Exhibitor" && $type->name != "Exhibitor Comp" && $type->name != "Attendant" && $type->name != "Delegate")
                                <option value="{{ $type->id }}">{{ $type->name }}</option>
                            @endif
                        @endforeach
                    </select>
                </div>
                <button type="button" onclick="storeBadge({{ $event->id }})" class="btn btn-dark w-100">Add Badge</button>
                </form>
            </div>
          </div>
        </div>
